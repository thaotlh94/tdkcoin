...
*Setup instructions for a Gitian build of TdkCoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the TdkCoin
...
- [Building TdkCoin Core](#building-tdkcoin-core)
...
Clone the git repositories for tdkcoin and Gitian.
...
in the tdkcoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building TdkCoin Core
----------------

To build TdkCoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the tdkcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/tdkcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/XGoldCoin/tdkcoin
...
For example:
```bash
URL=https://github.com/thrasher-/tdkcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit tdkcoin=${COMMIT} --url tdkcoin=${URL} ../tdkcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit tdkcoin=${COMMIT} --url tdkcoin=${URL} ../tdkcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit tdkcoin=${COMMIT} --url tdkcoin=${URL} ../tdkcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the tdkcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../tdkcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/tdkcoin
SIGPATH=/some/root/path/tdkcoin-detached-sigs

./bin/gbuild --url tdkcoin=${BTCPATH},signature=${SIGPATH} ../tdkcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/tdkcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/tdkcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/tdkcoin-osx-build.assert
```
...
[TdkCoin/gitian.sigs.adc](https://github.com/XGoldCoin/gitian.sigs.adc/) repository, or if that's not possible create a pull
