...
#if defined(NDEBUG)
# error "TdkCoin cannot be compiled without assertions."
#endif
...
bool SendMessages(CNode* pto, CConnman& connman, const std::atomic<bool>& interruptMsgProc)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        if (!IsInitialBlockDownload() && pto->nNextLocalAddrSend < nNow) {
            AdvertiseLocal(pto);
            pto->nNextLocalAddrSend = PoissonNextSend(nNow, AVG_LOCAL_ADDRESS_BROTDKAST_INTERVAL);
        }
        ...
        if (pto->nNextAddrSend < nNow) {
            pto->nNextAddrSend = PoissonNextSend(nNow, AVG_ADDRESS_BROTDKAST_INTERVAL);
            std::vector<CAddress> vAddr;
        ...
        {
            LOCK(pto->cs_inventory);
            vInv.reserve(std::max<size_t>(pto->vInventoryBlockToSend.size(), INVENTORY_BROTDKAST_MAX));
            ...
            if (pto->nNextInvSend < nNow) {
                ...
                pto->nNextInvSend = PoissonNextSend(nNow, INVENTORY_BROTDKAST_INTERVAL >> !pto->fInbound);
            }
            ...
            if (fSendTrickle) {
                ...
                LOCK(pto->cs_filter);
                while (!vInvTx.empty() && nRelayedTransactions < INVENTORY_BROTDKAST_MAX) {
        ...
        if (pto->nVersion >= FEEFILTER_VERSION && GetBoolArg("-feefilter", DEFAULT_FEEFILTER) &&
            ...
            if (timeNow > pto->nextSendTimeFeeFilter) {
                ...
                pto->nextSendTimeFeeFilter = PoissonNextSend(timeNow, AVG_FEEFILTER_BROTDKAST_INTERVAL);
            }
